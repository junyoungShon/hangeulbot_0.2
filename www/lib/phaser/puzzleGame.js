/**
 * Created by jyson on 2016. 7. 26..
 */

var deviceWidth = document.getElementById('contentDIV').offsetWidth;
var profileInContentDIVWidth = document.getElementById('profileInContentDIV').offsetWidth;
//var deviceHeight = document.getElementById('contentDIV').offsetHeight;
var deviceHeight = document.body.offsetHeight;

var game = new Phaser.Game(deviceWidth, deviceHeight-50, Phaser.CANVAS, 'phaser-example', { preload: preload, create: create });

var PIECE_WIDTH = deviceWidth/4,
  PIECE_HEIGHT = (deviceHeight-50)/3,
  BOARD_COLS,
  BOARD_ROWS;
...
function preload() {
  console.log('디바이스 높이 '+deviceHeight+" 디바이스 넓이" +deviceWidth)
  game.add.sprite(game.world.width,game.world.height,'background')
  game.load.spritesheet("background", "http://placehold.it/1920x1080", PIECE_WIDTH, PIECE_HEIGHT,Math.floor(game.world.width / PIECE_WIDTH)*Math.floor(game.world.height / PIECE_HEIGHT));
  game.world.width
}
...
function movePiece(piece, blackPiece) {
  ...
  game.add.tween(piece).to({x: blackPiece.posX * PIECE_WIDTH, y: blackPiece.posY * PIECE_HEIGHT}, 100, Phaser.Easing.Linear.None, true);
